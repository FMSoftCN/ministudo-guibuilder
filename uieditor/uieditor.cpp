...
void UIEditor::executeCommand(int cmd_id, int status, DWORD param)
{
		...
		if (IDOK == mng.DoMode())
		{
			if (!def_clt_font.empty())
			{
				...
				for(it = editors.begin(); it != editors.end(); ++it)
				{
					...
					for(cinst = cinst->getChildren(); cinst; cinst = cinst->getNext())
					{
						...
						{
							::SetWindowFont(cinst->getPreviewHandler(), clt_font);
						}
					}
				}
			}
			...
			if (exFontList.size() > old_size){
				...
				for (it = exFontList.begin();it != exFontList.end(); it++, i++)
				{
					if (i >= old_size){
						//copy
						string tmpfile = (*it)->file_name.substr((*it)->file_name.find_last_of('/'), -1);
						string de_file = g_env->getProjectPath();
						de_file += tmpfile;
						if (copyfile((*it)->file_name.c_str(), de_file.c_str()))
						{
							(*it)->file_name.clear();
							(*it)->file_name.append(de_file);
						}
						//load
						::LoadDevFontFromFile((*it)->font_name.c_str(), (*it)->file_name.c_str());
					}
				}
			}
			enableMenuItem(GBC_SAVE,TRUE);
	...
	else if(cmd_id == UI_MENUCMD_REMOVE_CUR )
	{
		if(!curEdit)
			return;
		//close
		int id = YesNoCancelBox(_("Delete"),_("Do You Want Delete \"%s\" From Disk?"), curEdit->getXMLFile());
...
BOOL UIEditor::setResId(int oldId, int newId, DWORD res/*=0*/)
{
	...
	if(newres){
		InfoBox(_("Error"), _("This ID is exist, please select another"));
		return FALSE;
	}
	...
	else //control
	{
		...
		if(strNewName != NULL){
			InfoBox(_("Error"), _("This ID is Pre Defined ID,Cannot use! If Your want use this ID, Please Change Your ID Name As \"%s\""),strNewName);
			return FALSE;
		}
	...
	if(node) {
		...
		for(node=node->children; node; node = node->next)
		{
			if(xhIsNode(node, "def-renderer")) {
				...
				::LoadDevFontFromFile(font->font_name.c_str(), font->file_name.c_str());
			}
		}
	}
...
BOOL UIEditor::saveConfig(TextStream* stream)
{
	...
	stream->println("<def-renderer>%s</def-renderer>", g_env->getDefRdrName());
	...
	stream->indent();
	if (!def_cap_font.empty())
		stream->println("<caption>%s</caption>", def_cap_font.c_str());
	if (!def_clt_font.empty())
		stream->println("<client>%s</client>", def_clt_font.c_str());
	stream->unindent();
	stream->println("</def-font>");
	...
	if (!exFontList.empty()){
		...
		for (it = exFontList.begin();it != exFontList.end(); it++)
		{
			stream->println("<import-font name=\"%s\" file=\"%s\" />",
					(*it)->font_name.c_str(), (*it)->file_name.c_str());
		}
	}
...
void UIEditor::saveExtends(TextStream *stream)
{
	...
	if(dir_extend_templates.size() > 0)
		stream->println("<extend-templates>%s</extend-templates>",dir_extend_templates.c_str());
	for(vector<string>::iterator it = extend_mainwnds.begin();
			it != extend_mainwnds.end(); ++it)
	{
		string &str = *it;
		if(str.size() > 0)
			stream->println("<extend-mainwnd>%s</extend-mainwnd>", str.c_str());
	}
	stream->unindent();
	stream->println("</extends>");
}
...
///////////////////////////////////////////////////////
static CTRLDATA _startwnd_templ_ctrls[] = {
	{
		CTRL_STATIC,
		WS_VISIBLE,
		10, 10, 180, 25,
...
static DLGTEMPLATE _startwnd_templ_tmpl = {
		WS_BORDER|WS_CAPTION|WS_DLGFRAME,
		WS_EX_NONE,
		300,200,300,250,
		_("Select Start Window ...."),
		0,0,
		sizeof(_startwnd_templ_ctrls)/sizeof(CTRLDATA),
		_startwnd_templ_ctrls,
		0
};
...
int UIEditor::selectStartWnd()
{
	...
	if(!DialogBoxIndirectParam(&_startwnd_templ_tmpl, m_hWnd,_startwnd_proc,(DWORD)&info))
		return -1;
...
static int _rdrwnd_proc(HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_COMMAND:
		...
		case IDOK:
		{
			...
			if(idx < 0)
			{
				MessageBox(hwnd, _("Please Select Default Renderer"), _("Error"), MB_OK);
				break;
			}
...
BOOL UIEditor::callSpecial(const char* strName, ...)
{
	...
	CALL_FUNCTION(strName, "updateRdrElement",
			VCALL2(updateRdrElement, Instance* ,int , va))
...
void UIEditor::updateRdrElement(Instance *inst, int ele_id)
{
	//update ALL
	for(mapex<string, EditUIPanel*>::iterator it = editors.begin();
			it != editors.end(); ++it)
	{
		it->second->updateRdrElement(inst, ele_id);
	}
}
